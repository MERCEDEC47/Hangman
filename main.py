...
#  получения слова из списка word_list
...
def proverka_simvola(que):
    while que.isalpha() == False:                       # Проверка является ли input буквой или словом
        que = input('Похожу что ты ввел не букву, попробуй снова  ')
...
def play(word):
    ...
    guessed_words = []
    tries = 6
    print('Давай сыграем в игру "Виселица"!')
    print(display_hangman(tries))
    print(word_completion, 'Я загадал слово из', len(word), 'букв.')

    while tries != 0:
        guess = input('Введите букву или слово целиком: ').upper()
        proverka_simvola(guess)
        ...
        if len(guess) == 1 and guess not in guessed_letters:
        ...
        elif len(guess) > 1 and guess not in guessed_words:
            tries -= 1
            ...
            print('Вы уже угадывали это слово или букву!')
